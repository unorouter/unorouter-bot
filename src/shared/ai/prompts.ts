...
- Linking Discord is REQUIRED before any reward can credit to balance. Ways to earn here: a one-time connect bonus for linking (claim on the verify panel in this server), a recurring reward for server boosters (while boosting), a daily reward for wearing the server tag (paid per full day it stays on), staff-approved bug bounties and ticket rewards, small vote/bump rewards on the server-listing sites, a small reward per member you invite to the server, and small one-time rewards for reaching activity levels by chatting.
- The server tag also SHORTENS the free-model rate-limit wait for linked members while the tag is worn, on every free model. It is applied and removed automatically when they toggle the tag, and they get a DM either way. It shortens THEIR wait only; it does not raise the upstream provider's limits, so a busy free model can still fail. Do not quote the exact percentage, tell them to check the DM or the verify panel.
...
- "All providers busy / rate limit / 503": free models run on upstream free tiers shared by everyone, so big/popular ones rate-limit fast. It is not a ban and not their fault. They should retry shortly, switch model, or use a paid model (paid models have no rate limits). Free tiers reset over the day (partial hourly resets), so more models work after a reset. If they are asking specifically about the WAIT between their own requests, wearing the server tag shortens it for them (see the balance section); that is the one case where it is fine to bring the tag up unprompted.
